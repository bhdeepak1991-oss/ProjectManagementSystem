...
function OnWebTwainNotFoundOnWindowsCallback(ProductName, InstallerUrl, bHTML5, bIE, bSafari, bSSL, strIEVersion) {  
    ...
    if (_isBothUtilityInstalled) {
        $(".save").prop('disabled', true);
        $("#btnSaveUserPrinterSetting").prop('disabled', true);
        
    } else {
        $(".save").prop('disabled', false);
        $("#btnSaveUserPrinterSetting").prop('disabled', false);
    }
    var hdnCurrentPage = $("#hdnCurrentPage").val();
    if (hdnCurrentPage == "PrinterSetup") {
        $('#dvScannerInstallation').show();
        $('#ddlScannerList').hide();
        //$("#dvPrinterGridForStore").removeClass("ajaxLoading");
    }
    else if (hdnCurrentPage == "GetAllPrescriptions") {
        OpenScannerModelOnFly();
       // _show_install_dialog_custom();
    }
    else if (hdnCurrentPage == "Layout") {
        $('#dvScannerInstallationForUser').show();
        $('#ddlScannerListForUser').hide();
        $("#dvScannerGridForUser").removeClass("ajaxLoading");
    }
    $("#dvScannerGridForStore").removeClass("ajaxLoading");
...
function DWT_Reconnect() {
    if (((new Date() - reconnectTime) / 1000) > 30) {
        ...
        if (el) {
            el.innerHTML = '<span class="failed2Connect">Failed to connect to the service, have you run the setup?<br />If not, please run the setup and <a href="javascript:void(0)" onclick="DCP_DWT_onclickInstallButton()">click here to connect again</a>.</span>';
        }
        return;
    }
    ...
    if (btnInstall) {
        ...
        setTimeout(function () {
            ...
            if (el && el.getAttribute("html5") == "1") {
                ...
                newDiv.innerHTML = '<span class="connectingService">Connecting to the service....</span>';
...
function _show_install_dialog(ProductName, objInstallerUrl, bHTML5, iPlatform, bIE, bSafari, bSSL, strIEVersion) {
    ...
    objInstallerUrl.default = objInstallerUrl.default.replace("DynamsoftServiceSetupTrial.msi", "DynamicWebTWAIN15.1Trial.zip");
    ...
    ObjString.push('<div class="btn-outer"><a id="dwt-btn-install" target="_blank" href="');
    ...
    console.log(objInstallerUrl['default']);
    if (bHTML5) {
        if (bIE) {
...
function _show_install_dialog_custom() {
    var ObjString, title;
    title = 'Scanner setup';
    ObjString = ['<div class="dynamsoft-dwt-dlg-title-custom">', title, '</div>'];
    //ObjString.push('<div class="dynamsoft-dwt-installdlg-iconholder"> ');


    ObjString.push('<div id="NoScannerFound"></div><br />' +
        '<div id="dvScannerInstallation">' +
        '<span>The Scanner Utility is not installed for this application.</span> <br />' +
        '<span>To install, click on Download Setup and manually install as per the instructions provided in the <a id="btnDownloadScannerManual" target ="_blank" href ="#">Installation Guide</a>.</span> <br />' +
        '<a id="downloadLinkScanExe" target="_blank" href="#"> <button type="button" class="btn btn-default btn-sm installSDK" id="btnInstallSDk">Download Setup</button> </a>' +
        '</div >');
...
$(document).delegate("#btnDownloadScannerManual", "click", function () { 
    var URI = window.location.href;
    var parser = document.createElement('a');
    parser.href = URI;

    var scrForDocsScan = parser.protocol + '//' + parser.host + "/js/Resources/dist/Scanner_Installation_Guide.pdf";
    $(this).attr('href', scrForDocsScan);
    $(this).click();
});

$(document).delegate("#btnInstallSDk", "click", function () { 
    var URI = window.location.href;
    var parser = document.createElement('a');
    parser.href = URI;

    var scrForExeScan = parser.protocol + '//' + parser.host + "/js/Resources/dist/PMSScannerUtility15.1.zip";
    $(this).parent().attr('href', scrForExeScan);
    $(this).parent().click();
});